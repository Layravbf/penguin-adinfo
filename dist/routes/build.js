        ...
        configDAO
            ...
            return new Promise((resolve, reject) => {
                converter.json2csv(jsonParameterized, (err, csv) => __awaiter(void 0, void 0, void 0, function* () {
                    ...
                    const linesToSaveIntoCsv = csvArrayContent.map((csvLine) => csvLine.split(separator).slice(0, -1).join(separator));
                    if (fullHistoricalContent) {
                        ...
                        const csvContentNewField = linesToSaveIntoCsv.map((csvLine) => `${fileName}.csv${separator}${csvLine}`);
                        ...
                        correctHistoricalContent +=
                            '\n' +
                                linesCorrectToSaveIntoCsv
                                    .map((csvLine) => `${fileName}.csv${separator}${csvLine}`)
                                    .join('\n');
                    }
                    else {
                        const csvArrayHeaderNewField = 'Arquivo' + separator + csvHeader;
                        const csvContentNewField = linesCorrectToSaveIntoCsv.map((csvLine) => `${fileName}.csv${separator}${csvLine}`);
