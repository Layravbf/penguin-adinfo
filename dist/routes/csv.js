...
const csv = (app) => {
	app.post('/csv', (req, res) => {
		...
		const filePath = adOpsTeam
			? `${advertiser}/${adOpsTeam}/${campaign}/${DateUtils_1.DateUtils.generateDateString()}.csv`
	...
	app.get('/csv', (req, res) => {
		...
		const campaign = req.headers.campaign;
		const advertiser = req.advertiser;
		const adOpsTeamPath = req.headers.adopsteam ? req.headers.adopsteam : 'AdvertiserCampaigns';
		const apiResponse = new ApiResponse_1.ApiResponse();
		if (!fileName) {
	...
	app.get('/csv/list', (req, res) => {
		...
		const apiResponse = new ApiResponse_1.ApiResponse();
		if (adOpsTeam) filePath += `${adOpsTeam}/`;
