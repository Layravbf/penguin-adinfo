...
class Builder {
    ...
    build() {
        ...
        const linesBuilded = linesWithContent.map((lineFromFile) => {
            const parametrizerObject = new ParametrizerFactory_1.ParametrizerFactory(lineFromFile, this.advertiserConfig).build(this._analyticsTool);
            const parameters = parametrizerObject.buildedLine();
            if (this._media) {
                const mediaParameters = new ParametrizerFactory_1.ParametrizerFactory(lineFromFile, this.advertiserConfig)
