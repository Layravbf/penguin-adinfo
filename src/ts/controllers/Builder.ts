...
export class Builder {
	...
	private advertiserConfig: Config;
	...
	public build(): { [key: string]: any }[] {
		...
		const linesBuilded: { [key: string]: string }[] = linesWithContent.map((lineFromFile) => {
			const parametrizerObject = new ParametrizerFactory(lineFromFile, this.advertiserConfig).build(
				this._analyticsTool
			);
			const parameters = parametrizerObject.buildedLine();
			if (this._media) {
				const mediaParameters = new ParametrizerFactory(lineFromFile, this.advertiserConfig)
